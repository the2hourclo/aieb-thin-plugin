...
- `hooks/`: onboarding, roadmap, update, and retrospective nudges; a skill-edit guard that steers edits of served skills toward overrides; a local next-session win/friction detector that reads user messages only; and a one-time end-of-session outcome-report nudge.
- `skill-telemetry/`: explicit one-note feedback only; no automatic transcript capture or consent prompt.

No transcript, prompt, uploaded file, memory, or customer business data is stored in this shell or sent as analytics. Business content stays in the buyer's local workspace. Server events are limited to pseudonymous member/device references, skill IDs, fixed outcome/feedback categories, versions, latency, and estimated token counts.

## Feedback coverage across hosts

The universal layer is the MCP itself, because not every CLI or desktop app exposes lifecycle hooks:

| Host capability | What collects wins and friction |
|---|---|
| Any app using the AIEB MCP | Every served entry skill carries the enum-only `report_product_outcome` and `report_skill_feedback` contract. Explicit praise/acceptance and correction/rejection map to fixed categories; no quoted feedback or work product is accepted. |
| Claude Code / Codex / Agent Plugins hosts with hooks | The shared Stop hook catches a missing terminal outcome once. The SessionStart hook scans the prior transcript locally for explicit user win/friction language and nudges the agent once. |
| Hookless MCP hosts | The MCP contract still works through initialize instructions, dynamic tool descriptions, and the reminder embedded in every entry-skill response. There is no transcript interceptor to install, so the model files the structured signal when the user gives it. |

Rich improvement notes are separate: they are genericized locally and sent only after the user explicitly approves that one note. The categorical MCP events never contain free text and reuse the existing `aieb_events` ledger rather than creating another database.
