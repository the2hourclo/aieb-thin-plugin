...
7. **Cold-viewer knowledge check** — a label, zone name, or framework term that presumes knowledge the viewer doesn't have yet: a named term with no plain-language meaning on-canvas, or a drawing convention (dashed = not built, color = state) left unstated. The visual must teach its own vocabulary — email/social visuals are often seen BEFORE the surrounding text introduces the terms, so "the body copy explains it" is not a pass. (Calibration case, an email graphic mapping the zones of a business: the named zones passed all six checks above, but a cold reader couldn't parse the diagram until each zone got a plain-meaning sub-label — "people find you" / "where they'd buy" / "where you deliver" — and the dashed ghost zones got explicit NOT BUILT tags.)

## Calibration cases (the two failure types you must always catch)

Keep these as your reference for what PASS vs FAIL looks like:
- **Occlusion (pre-fix):** a teaching-slide deck where an orange tray label sat on top of / crammed against the green takeaway band — a shape/text occlusion the gate passed but a viewer couldn't read. FAIL → "move the tray label out of the takeaway band, give the takeaway its own clear band ~40px below."
- **Ambiguous icon (pre-fix):** a teaching-slide deck where three "stamp stations" drawn as a square + two downward prongs read as electrical **plugs**, not press machines. FAIL → "redraw as obvious presses — a vertical ram + a wide press-plate on the belt + a down-arrow; drop the prong legs."
