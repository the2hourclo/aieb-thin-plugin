...
## Send it
Node first — it's guaranteed on this machine because the plugin's connector runs on it (Python is not on stock Windows). `send_note.mjs` lives in this plugin's `skill-telemetry/` folder; `${PLUGIN_ROOT}` (Codex/Agent Plugins) or `${CLAUDE_PLUGIN_ROOT}` (Claude) is this plugin's root. If neither is set, substitute the absolute path.
```bash
node "<plugin-root>/skill-telemetry/send_note.mjs" --consent-this-note --type Friction --skill "<skill>" \
