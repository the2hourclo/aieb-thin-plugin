---
name: excali-graphic
description: "Writing employee · Render legible hand-drawn 'Excalidraw whiteboard' diagrams, graphs, and charts as PNGs from hand-authored HTML/SVG — cream background, real Excalifont, an SVG wobble filter on SHAPES ONLY so text stays crisp, color-by-meaning accents. The go-to recipe for email graphs, newsletter diagrams, process maps, and video teaching slides. USE WHEN user says 'make a graph for my email', 'excalidraw diagram', 'whiteboard graphic', 'hand-drawn diagram', 'diagram this', 'make a chart', 'draw a process diagram', 'visualize this as a graph', 'teaching-slide graphic', 'sketch this concept on a board', or wants a clean hand-drawn diagram/graph for a video slide, email, newsletter, carousel slide, or canvas. Do NOT use for: photoreal/AI-generated imagery (outside this plugin's scope), or a full multi-slide social carousel (→ carousel — though its individual diagram slides can be drawn HERE)."
---

# excali-graphic — MCP loader
...
- *"read `references/foo.md`"* → `get_skill(skill_id="excali-graphic", path="references/foo.md")`
- *"load `workflows/bar.md`"* → `get_skill(skill_id="excali-graphic", path="workflows/bar.md")`
