...
| Finding | Offer |
|---|---|
| Managed block missing or `v=` stale | Refresh ONLY the text between the `:start` and `:end` markers with the current template (re-fetch `onboard` — workflow `workflows/scaffold-workspace.md` — for the canonical block text). Never touch anything outside the markers; show what changes first. |
...
| Key or connection problems on ANY device/chat | The fix is the same everywhere: run `/setup-aieb` (or say "set up my AI Employee Builder connection") — it gives a secure one-time link where they enter their license key on the activation PAGE. **The key never goes into chat.** Their key is in their Lemon Squeezy receipt email. (A connector older than v0.13 lacks the secure link flow — update the plugin first.) |
