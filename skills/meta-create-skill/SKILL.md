...
description: "Builder employee · Create, validate, test, upgrade, and improve Claude skills — and chain them into Skill Systems (AI Employees) — and decide WHICH of those an outcome actually needs. Use this when someone describes an OUTCOME rather than a build: 'I want AI to review my emails every week', 'can AI handle X for me', 'I want this to happen automatically', 'every week I have to…', 'I keep doing this manually' (route to references/artifact-selection.md FIRST — the answer is often a schedule on an existing skill, or a connector, not a new skill). Also use when someone says 'create a skill', 'build a skill for X', 'add a skill', 'turn this into a skill', 'build a skill system', 'build an AI employee', 'automate this whole operation', 'turn this process into a system', 'validate skill', 'test skill', 'improve this skill', 'update this skill', 'update skill X', 'add this to the skill', 'add a rule/principle to a skill', 'include this in our skills', 'the skill should also', 'remember this in the skill', 'change how this skill works', 'fix the routing', 'upgrade my skills', 'audit my skill library', 'harden this', 'make this reliable', 'make this production-grade', 'enforce these rules', 'it keeps ignoring its own rule', 'it keeps making the same mistake', 'it said it did it but it didn't', 'it works sometimes and not others', 'I keep having to remind it', 'schedule this skill', 'run it automatically every morning', 'make it recurring', 'automate this so I don't have to ask', 'build a taste harness', 'make it decide like me', 'decision tree for the copy/avatars/angles', 'it blends the options instead of picking', 'the output is average even though the research is good', or ANY time they want to change an existing skill or extend Claude's capabilities. ALWAYS load this skill before editing any `.claude/skills/**` file — a direct hand-edit skips the evidence-driven discipline (origin + carve-out + mechanical→gate + version bump) this skill enforces. This is the primary skill-authoring toolkit in ai-employee-builder — use it liberally; skill-creation, skill-updates, system-building, and skill-cleanup requests are exactly what it's for. A skill that MISFIRED and needs a patch from that observed failure routes to retrospective instead."
---
