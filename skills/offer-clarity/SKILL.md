---
name: offer-clarity
description: "Writing employee · Decide, diagnose, and independently review offers. USE WHEN user says 'roast my offer', 'clarify my offer', 'why isn't this selling', 'reposition this offer', 'what should I sell', 'what offer to sell', 'I do not know what offer to sell', 'three pillar exercise', 'run the rulebook', 'check this low-ticket offer', or 'offer QA'. Runs in fresh context to avoid self-grading. Do NOT use to write campaign assets or offer documents → `copywriter`; generic website copy → `copywriting` if available, otherwise return an evidence-backed page-copy brief for the user's installed producer."
context: fork
agent: general-purpose
effort: high
---

# offer-clarity — MCP loader
...
- *"read `references/foo.md`"* → `get_skill(skill_id="offer-clarity", path="references/foo.md")`
- *"load `workflows/bar.md`"* → `get_skill(skill_id="offer-clarity", path="workflows/bar.md")`
